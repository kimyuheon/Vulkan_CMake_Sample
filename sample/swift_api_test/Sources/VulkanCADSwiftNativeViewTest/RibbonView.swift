...
final class RibbonView: NSView {
    ...
    private func setup() {
        ...
        groupRow.spacing = 6
        groupRow.edgeInsets = NSEdgeInsets(top: 6, left: 10, bottom: 4, right: 10)
        ...
        NSLayoutConstraint.activate([
            ...
            groupRow.bottomAnchor.constraint(equalTo: scroll.contentView.bottomAnchor),
    ...
    /// 리본 큰 버튼 — 아이콘 위, 글자 아래. 마우스를 올렸을 때만 테두리.
    private func makeButton(_ tool: CADTool) -> NSButton {
        let btn = NSButton(title: tool.title, image: NSImage(), target: target, action: action)
        ...
        NSLayoutConstraint.activate([
            btn.widthAnchor.constraint(greaterThanOrEqualToConstant: 58),
